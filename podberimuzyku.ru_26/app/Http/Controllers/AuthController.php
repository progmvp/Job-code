...
class AuthController extends Controller
{
    ...
    public function sendCode(Request $request)
    {
         ...
         *
         * Пользователь без аккаунта ЛК не создаётся автоматически.
         */
        ...
        /*
         * Проверяем действующую подписку.
         */
        if (
            !$user->subscription_expires_at ||
            now()->greaterThanOrEqualTo($user->subscription_expires_at)
        ) {
            return back()
                ->withInput()
                ->withErrors([
                    'email' => 'Действующая подписка не найдена.',
                ]);
        }

        /*
         * Не позволяем запрашивать новый код слишком часто.
         */
        ...
        /*
         * Сохраняем только хеш кода.
         * Сам код в сессии не хранится.
         */
        session([
        ...
        /*
         * Отправляем транзакционное письмо через настроенный Laravel Mail.
         */
        Mail::raw(
    ...
    public function verifyCode(Request $request)
    {
        ...
        /*
         * На момент фактического входа снова проверяем подписку.
         */
        if (
            !$user->subscription_expires_at ||
            now()->greaterThanOrEqualTo($user->subscription_expires_at)
    ...
    public function dashboard()
    {
        ...
        /*
         * Дополнительная проверка подписки при каждом входе в кабинет.
         */
        if (
            !$user->subscription_expires_at ||
            now()->greaterThanOrEqualTo($user->subscription_expires_at)
            ...
            return redirect()
                ->route('lk.login')
                ->withErrors([
                    'email' => 'Срок действия подписки закончился.',
                ]);
        }
